...
const Students = ({ setIsLogin }) => {
  ...
  const validate = (data) => {
    ...
    if (!data.profession) {
      errors.profession = 'Cargo es requerido.';
  }
    ...
    try {
      ...
      if (token) {
        if(validate(user).isValidated){
          ...
          user.graduate = user.graduate.map((item)=>{  return item._id })
  ...
  const setDataUser = async (id, index) => {
    ...
    console.log(respuesta.data.graduate)
    setEditar(true);
    setUserDialog(true);
    setIdEditar(id);
    setError('');
          ...
          <div className='card'>
            <div className='card-body'>
              <div className='datatable-crud-demo'>
                ...
                <DataTable
                  ref={dt}
                  value={datos.filter(f=> f.role === 'student')}
                  dataKey='id'
                  paginator
                  rows={8}
                  showGridlines
